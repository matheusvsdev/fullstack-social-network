...
public class TestConfig {
    ...
    public void init() {
        ...
        ProfileEntity profileMaria = new ProfileEntity(null, "https://i.pinimg.com/736x/54/8a/65/548a659c2b06a877516d3c998f5b0939.jpg", "dev.maria", "Natal - RN", maria);
        ProfileEntity profileAlex = new ProfileEntity(null, "https://cdn-icons-png.flaticon.com/512/6858/6858485.png", "eng.alex", "São Paulo - SP", alex);
        ProfileEntity profileBob = new ProfileEntity(null, "https://cdn.icon-icons.com/icons2/2643/PNG/512/male_man_boy_black_tone_people_person_avatar_icon_159356.png","java.bob", "Rio de Janeiro - RJ", bob);
        ...
        PostEntity post1 = new PostEntity(null, "https://www.napratica.org.br/wp-content/uploads/2022/04/Foto-de-fauxels-no-Pexels-27.png", "Modelo de um código bem escrito(contém ironia)", Instant.now(), new Author(profileMaria));
        PostEntity post2 = new PostEntity(null, "https://safesea.com.br/midias/2021/10/marenatureza-1.jpg", "As férias começaram!", Instant.now(), new Author(profileMaria));
        ...
        Comment comment1 = new Comment(profileAlex.getObjectId(), profileAlex.getUsername(), "Boa viagem!", Instant.now().plusSeconds(1800));
        Comment comment2 = new Comment(profileBob.getObjectId(), profileBob.getUsername(), "Aproveite!", Instant.now().plusSeconds(900));
        Comment comment3 = new Comment(profileAlex.getObjectId(), profileAlex.getUsername(), "Tenha um ótimo dia!", Instant.now().plusSeconds(4200));
